...
public class RNSoundPoolModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void addSoundOffset(String name, int length, Promise promise){
        if(!soundMap.containsKey(name)) {
            ...
            if(afd != null) {
                ...
                int i = sp.load(afd.getFileDescriptor(), afd.getStartOffset(), 2000L, 1);
                promises.put(i, promise);
                soundMap.put(name, i);
            }
        }
